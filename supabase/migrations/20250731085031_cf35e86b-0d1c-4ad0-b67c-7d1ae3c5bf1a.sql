...
INSERT INTO public.reviews (cafe_id, user_id, title, content, rating)
SELECT 
  c.id,
...
CROSS JOIN (
  VALUES 
    ('Amazing coffee!', 'This place has the best espresso in town. The atmosphere is perfect for working.', 5),
    ('Great atmosphere', 'Love the cozy vibe and friendly staff. Coffee is excellent too!', 4),
    ('Perfect for meetings', 'Professional environment with great WiFi and comfortable seating.', 4),
    ('Delicious food', 'Not just great coffee, but the pastries and light meals are fantastic.', 5)
) AS review(title, content, rating)
LIMIT (SELECT COUNT(*) * 2 FROM public.cafes); -- 2 reviews per cafe